...
/**
 * Builds the unsigned `settle_order` transaction for the buyer to sign:
 * "I received what I paid for - release my escrowed funds to the seller."
 * This is the missing half of the checkout lifecycle the UI could already
 * start (fund) but never finish. The on-chain program enforces that only
 * the order's recorded buyer can settle; this route just builds the tx.
 */
export async function POST(
  ...
  if (status !== "FUNDED") {
    return NextResponse.json(
      { message: `order is ${status}, only a FUNDED order can be settled` },
      { status: 409 }
    );
  }
  ...
  const settleIx = await program.methods
    .settleOrder(marketItemIdToBn(marketItemId))
    .accountsPartial({
      buyer,
      seller,
      orderState: orderPda,
  ...
  // Buyer pays the (tiny) rent if the seller somehow has no ATA yet -
  // acceptable for the buyer-signs flow, and idempotent when it exists.
