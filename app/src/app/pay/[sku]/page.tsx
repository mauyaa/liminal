...
/** "12 deals · 0 disputes" / "New seller — first deal" beside the checkout header. */
function SellerHistory({ sellerWallet }: { sellerWallet?: string }) {
  const [deals, setDeals] = useState<number | null>(null);

  useEffect(() => {
    if (!sellerWallet) return;
    fetch(`/api/merchant/stats?merchantWallet=${sellerWallet}`)
      .then((res) => res.json())
      .then((body) => setDeals(body?.ordersByStatus?.SETTLED ?? 0))
      .catch(() => setDeals(null));
  }, [sellerWallet]);
  ...
  return (
    <p className="text-[11px] text-muted">
      {deals === 0 ? "New seller — first deal" : `${deals} deal${deals === 1 ? "" : "s"} · 0 disputes`}
    </p>
  );
}
      ...
      <main className="route-main max-w-[1040px]">
        ...
        {listing && <>
          <header className="flex items-center justify-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold tracking-[-.02em]">Liminal</p>
              <p className="text-[11px] text-muted">Protected checkout</p>
              <SellerHistory sellerWallet={listing.sellerWallet} />
            </div>
          </header>
            ...
            <div className="grid lg:grid-cols-[1.08fr_.92fr]">
              <div className="p-7 sm:p-11 lg:border-r lg:border-border">
                ...
                <dl className="mt-5 text-[12px]">
                  <div className="flex justify-between gap-6 py-3"><dt className="text-muted">Your money is</dt><dd className="font-semibold">Locked, not sent</dd></div>
                  ...
                  <div className="flex justify-between gap-6 py-3"><dt className="text-muted">If delivery is unconfirmed</dt><dd className="font-semibold">Auto-refunded after {windowLabel(listing.deliveryWindowSeconds)}</dd></div>
