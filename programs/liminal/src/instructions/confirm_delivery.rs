...
/// Lets the buyer release funds early once delivery has been signaled,
/// instead of waiting out the full challenge window. Identical transfer
/// body to `settle_order` - the only difference is which status it requires
/// (`DeliverySignaled` instead of `Funded`), kept as a separate instruction
/// rather than widening `settle_order`'s own check since existing
/// instruction logic stays untouched.
#[derive(Accounts)]
...
pub fn handle_confirm_delivery(ctx: Context<ConfirmDelivery>, _market_item_id: u64) -> Result<()> {
    require!(
