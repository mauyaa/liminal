...
- A Solana Actions/Blinks checkout API and a couple of small merchant/admin
  endpoints (`app/src/app/api`), backed by a Drizzle/libSQL schema.
...
app/                      Next.js app: Actions/Blinks API + DB schema
...
- `tsc --noEmit`, `eslint .`, and `next build` all clean in `app/`.
